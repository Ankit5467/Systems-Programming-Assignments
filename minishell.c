 ...
 * Author      : Ankit Patel
 * Date        : 4/14/2021
 * Description : Minishell programming assignment. Implementation of a shell with minimal
					features such as cd, executing a binary, and signal catching.
					Note: the execution feature is implemented using execvp and forking.
...
int main(int argc, char *argv[]) {
	...
	while (true) { /* take in & process commands until user exits */
		...
		if (read(STDIN_FILENO, inp, PATH_MAX) < 0) {
			/* don't EXIT_FAILURE if read failed because of SIGINT */
			if (!interrupted) {
